...
class State {
    constructor(name, config = default_config_1.defaultConfig) {
        this.name = name;
        this.config = config;
        ...
        this.dispatchCommand = p_throttle_1.default(this.dispatchCommand.bind(this), this.config.ioThrottleFrequency, 1000);
    }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const [uri, event] of this.lastEvents) {
                if (predicate(event.nextState)) {
                    const nextEvent = yield this.dispatchCommand(uri, commandType);
                    events.push(nextEvent);
                    onProgress && onProgress(nextEvent);
                }
            }
