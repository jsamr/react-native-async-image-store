...
class OfflineImage extends react_1.PureComponent {
    ...
    render() {
        const _a = this.props, { source, ImageComponent: Img = react_native_1.Image, LoadingIndicatorComponent = react_native_1.ActivityIndicator, fallbackStaticSource: fallbackSource, storeName } = _a, imageProps = __rest(_a, ["source", "ImageComponent", "LoadingIndicatorComponent", "fallbackStaticSource", "storeName"]);
        const { fileState, syncState, localURI } = this.state;
        const LoadingComponent = LoadingIndicatorComponent;
        ...
        if (displayFallback && fallbackSource) {
            return react_1.default.createElement(ImageComponent, Object.assign({ source: fallbackSource }, imageProps));
        }
        if (loading || displayFallback) {
            return react_1.default.createElement(LoadingComponent, Object.assign({}, imageProps));
        }
