...
class State {
    ...
    getLocalURIForRemoteURI(remoteURI) {
        const pathLens = ramda_1.lensProp('localURI');
        const pathFromURI = ramda_1.view(pathLens, this.getURIModel(remoteURI));
    ...
    getFileNamePrefixForURI(remoteURI) {
        return buffer_1.Buffer.from(remoteURI).toString('base64');
    }
    getFilePrefixURIForRemoteURI(remoteURI) {
        return utils_1.joinUri(this.getBaseDirURI(), this.getFileNamePrefixForURI(remoteURI));
    }
